...
function App() {
  ...
  const [electric_power_consumption, set_electric_power_consumption] = useState(null);
  const [forest_area, set_forest_area] = useState(null);
  const [gdp_per_capita, set_gdp_per_capita] = useState(null);
  const [individuals_using_the_internet, set_individuals_using_the_internet] = useState(null);
  const [military_expenditure, set_military_expenditure] = useState(null);
  const [people_practicing_open_defecation, set_people_practicing_open_defecation] = useState(null);
  const [people_using_at_least_basic_drinking_water_services, set_people_using_at_least_basic_drinking_water_services] = useState(null);
  const [obesity_among_adults, set_obesity_among_adults] = useState(null);
  const [beer_consumption_per_capita, set_beer_consumption_per_capita] = useState(null);
  ...
  const handleAddRecord = async (e) => {
    ...
    try {
      const res = await fetch("http://127.0.0.1:8000/api/people", {
        method: "POST",
  ...
  const inputsValid = () => {
    ...
    for (let i=0; i<formRef.current.length; i++) {
      if (formRef.current[i].id.startsWith("H") && formRef.current[i].validity.badInput) {
        const header = formRef.current[i].name;
        let errorMsg = `Bad input at ${header.split('_').map(word => word[0].toLocaleUpperCase() + word.substring(1)).join(' ')} - please provide valid `;
  ...
  const handleEdit = row => {
    ...
    set_life_expectancy(row.life_expectancy === '---' ? null : row.life_expectancy);
    set_population(row.population === '---' ? null : row.population);
    set_co2_emissions(row.co2_emissions === '---' ? null : row.co2_emissions);
    set_health_expenditure(row.health_expenditure === '---' ? null : row.health_expenditure);
    set_electric_power_consumption(row.electric_power_consumption === '---' ? null : row.electric_power_consumption);
    set_forest_area(row.forest_area === '---' ? null : row.forest_area);
    set_gdp_per_capita(row.gdp_per_capita === '---' ? null : row.gdp_per_capita);
    set_individuals_using_the_internet(row.individuals_using_the_internet === '---' ? null : row.individuals_using_the_internet);
    set_military_expenditure(row.military_expenditure === '---' ? null : row.military_expenditure);
    set_people_practicing_open_defecation(row.people_practicing_open_defecation === '---' ? null : row.people_practicing_open_defecation);
    set_people_using_at_least_basic_drinking_water_services(row.people_using_at_least_basic_drinking_water_services === '---' ? null : row.people_using_at_least_basic_drinking_water_services);
    set_obesity_among_adults(row.obesity_among_adults === '---' ? null : row.obesity_among_adults);
    set_beer_consumption_per_capita(row.beer_consumption_per_capita === '---' ? null : row.beer_consumption_per_capita);
  ...
  const fetchData = async () => {
    try {
      let res = await fetch("http://127.0.0.1:8000/api/people" + (search ? `?country=${search}`:''));
      if (res) {
        res = await res.json();
      ...
      res = await fetch("http://127.0.0.1:8000/api/people/beer_is_life");
      if (res) {
        res = await res.json();
        setDataBeer(res);
      }
  ...
  const data = useMemo(
    () => dataRaw.map(( item => 
      Object.fromEntries(HEADERS.map((H, id) => {
        if (item[H] === null) {
          return [H, '---']
        }
  ...
  const columns = useMemo(
    () => HEADERS.map(name => ({
      Header: name.split('_').map(word => word[0].toLocaleUpperCase() + word.substring(1)).join(' '),
      accessor: name
    })), []
  );
  ...
  return (
    <div className="App font-mont">
            ...
            <form ref={formRef} onSubmit={ handleAddRecord } >
              <div className="grid grid-cols-2 gap-2">
                ...
                {HEADERS.map((H, id) => {
                  ...
                  // type === str
                  let inputElement = <input id={`H${id}`} onFocus={handleFocus} type="text" name={H} value={eval(H) ?? ''} onChange={a => eval(`set_${H}('${a.target.value}')`)} className="border border-black h-8 px-2"/>

                  if (type === 'int') {
                    inputElement = <input id={`H${id}`} onFocus={handleFocus} type="number" min="0" step="1" name={H} value={eval(H) ?? ''} onChange={a => eval(`set_${H}(${a.target.value})`)} className="border border-black h-8 px-2"/>
                  } else if (type === 'float') {
                  ...
                  return (
                    <Fragment key={id}>
                      <label htmlFor={`H${id}`}>{H.split('_').map(word => word[0].toLocaleUpperCase() + word.substring(1)).join(' ')}</label>
                      {inputElement}
                    </Fragment>
                  )
                })}
              ...
              <div className="flex flex-row justify-between mt-4">
                <div className="flex flex-row gap-3">
                  <button onClick={ () => {setModalVisible(false); setError('')} } type="button" className="bg-gray-500 hover:bg-gray-600 px-4 py-2 rounded-xl text-white w-32">
                    Cancel
                  </button>
                  {isModalAddRecord && (
                    <button onClick={ handleReset } type="reset" className="bg-red-500 hover:bg-red-600 px-4 py-2 rounded-xl text-white w-32">
                      Reset
                    </button>
                  )}
                </div>
                {isModalAddRecord ? (
                  <button onClick={ handleAddRecord } type="submit" className="bg-blue-500 hover:bg-blue-600 px-4 py-2 rounded-xl text-white w-32">
                    Submit
                  </button>
                ) : (
                  <button onClick={ handleEditRecord } type="submit" className="bg-blue-500 hover:bg-blue-600 px-4 py-2 rounded-xl text-white w-32">
                    Edit record
                  </button>
                )}
      ...
      <div className="flex flex-col items-center py-10 gap-4 mx-8">
        <p className="font-bold text-4xl text-center">Life Expectancy 2000-2015</p>
        <a className="text-blue-500 hover:underline" href="https://www.kaggle.com/datasets/vrec99/life-expectancy-2000-2015">Link for data source</a>
      </div>
            ...
            { headerGroups.map(headerGroup => (
              <tr { ...headerGroup.getHeaderGroupProps()}>
                ...
                { headerGroup.headers.map(column => (
                    <th { ...column.getHeaderProps() } className="border border-black px-2 py-1">
              ...
              return (
                <tr { ...row.getRowProps() }>
                  <td className="border border-black px-2 py-1 whitespace-nowrap">
                    <button className="text-blue-500 hover:underline" onClick={() => handleEdit(row.original)}>
                      Edit
                    </button>
                  </td>
                  { row.cells.map(cell => (
                      <td {...cell.getCellProps()} className="border border-black px-2 py-1 whitespace-nowrap">
                        { cell.render('Cell') }
                      </td>
                  ))}
                  <td className="border border-black px-2 py-1 whitespace-nowrap">
      ...
      {dataBeer && (
        <div className="w-[400px] h-[400px] m-10">
          ...
          data={{
            ...
            datasets: [{
              data: [dataBeer.results, dataBeer.country_count - dataBeer.results],
              backgroundColor: [
                'rgba(75, 192, 192, 0.2)',
                'rgba(255, 159, 64, 0.2)',
              ],
